...
def Search(item):  # standard input
    ...
    if subtitles:
        for it in subtitles:
            ...
            listitem = xbmcgui.ListItem(label=it["language_name"],
                                        label2=label)
            listitem.setArt({'icon':str(int(round(float(it["rating"])/2)))})
            listitem.setArt({'thumb':it["language_flag"]})
    ...
    if item['original_title'] != "true":
        listitem = xbmcgui.ListItem(label="", label2=__language__( 32011 ).encode("utf-8"))
...
def xbmc_walk(DIR):
    ...
    for dir in dirs:
        LIST.extend(list(xbmc_walk(os.path.join( DIR, dir ).replace('\\','/'))))
    return LIST
    ...
    if len(subtitles) > 1:
        ...
        if sel >= 0:
            ...
            if subtitles[sel][0] == "pack":
                dir = os.path.dirname(urllib.parse.unquote(xbmc.Player().getPlayingFile()))
...
if params['action'] == 'search' or params['action'] == 'manualsearch':
    ...
    item["languages"].extend(urllib.parse.unquote(params['languages']).split(","))
    ...
    for lang in item["languages"]:
        if __search__ == '0':
            if lang == "Portuguese (Brazil)": langtemp.append((0, lang))
            elif lang == "Portuguese": langtemp.append((1, lang))
            elif lang == "English": langtemp.append((2, lang))
            else: langtemp.append((3, lang))
        elif __search__ == '1':
            if lang == "Portuguese (Brazil)": langtemp.append((1, lang))
            elif lang == "Portuguese": langtemp.append((0, lang))
            elif lang == "English": langtemp.append((2, lang))
            else: langtemp.append((3, lang))
        elif __search__ == '2':            
            if lang == "Portuguese (Brazil)": langtemp.append((1, lang))
            elif lang == "Portuguese": langtemp.append((2, lang))
            elif lang == "English": langtemp.append((0, lang))
            else: langtemp.append((3, lang))
        elif __search__ == '3':        
            if lang == "Portuguese (Brazil)": langtemp.append((1, lang))
            elif lang == "Portuguese": langtemp.append((2, lang))
            elif lang == "English": langtemp.append((3, lang))
