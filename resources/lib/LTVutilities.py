...
def log(msg, logtype="DEBUG"):
  ...
  try:
    xbmc.log(("### [%s] - %s" % (__scriptname__,msg,)).encode('utf-8'), level=loglevel )
  except:
    return None

def getTheTVDBToken():
    HTTPRequest = urllib.request.Request("https://api.thetvdb.com/login", data=json.dumps({"apikey" : TheTVDBApi}), headers={'Content-Type' : 'application/json'})
    try:
...
def getShowIMDB():
    ...
    if ShowID:
        try:
            ...
            HTTPRequest  = urllib.request.Request("https://api.thetvdb.com/series/%s" % ShowID, headers={'Authorization' : 'Bearer %s' % TheTVDBToken})
            HTTPResponse =  urllib.request.urlopen(HTTPRequest).read()
...
def getMovieIMDB():
    try:
        ...
        if "imdbnumber" in movieid['result']['item'] and len(movieid['result']['item']["imdbnumber"]):
            log("getMovieIMDB: IMDB id ->%s<- already on database." % movieid['result']['item']["imdbnumber"], "DEBUG" )
            return movieid['result']['item']["imdbnumber"]
        elif "title" in movieid['result']['item'] and len(movieid['result']['item']["title"]):
            MovieTitle             = movieid['result']['item']["title"].encode("utf-8");
            Query                  = urllib.parse.urlencode({"api_key" : TMDBApi, "page" : "1", "query" : MovieTitle, "year" : movieid['result']['item']["year"]})
            HTTPRequest            = urllib.request.Request("https://api.themoviedb.org/3/search/movie", data=Query)
            ...
            if len(JSONContent['results']):
                ...
                HTTPRequest  = urllib.request.Request("https://api.themoviedb.org/3/movie/%s?api_key=%s" % (TMDBId, TMDBApi))
                HTTPResponse = urllib.request.urlopen(HTTPRequest).read()
                JSONContent  = json.loads(HTTPResponse);
...
def getMovieOriginalTitle(Title, MovieID):
    if MovieID:
      HTTPRequest  = urllib.request.Request("https://api.themoviedb.org/3/find/%s?external_source=imdb_id&api_key=%s" % (MovieID, TMDBApi))
