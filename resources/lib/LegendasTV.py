...
def normalizeString(obj):
    try:
        return unicodedata.normalize('NFKD', str(obj)).encode('ascii', 'ignore').decode('ascii')
    except:
        return str(str(obj).encode('unicode_escape'))
...
class LegendasTV:
    ...
    def login(self, username, password):
        ...
        if self.cookie:
            ...
            opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(self.cookie))
            urllib.request.install_opener(opener)
            login_data = urllib.parse.urlencode({'_method':'POST', 'data[User][username]':username, 'data[User][password]':password})
            request = urllib.request.Request("http://legendas.tv/login", login_data.encode('utf-8'))
            response = normalizeString(urllib.request.urlopen(request).read())
            if response.__contains__('Usuário ou senha inválidos'):
                self.Log( " Login Failed. Check your data at the addon configuration.")
                return None
            else: 
                return self.cookie
    ...
    def _UNICODE(self,text):
        if text:
            return str(BeautifulSoup(text, fromEncoding="utf-8",  smartQuotesTo=None))
        else:
            return text
    ...
    def _log_List_dict(self, obj, keys="", maxsize=100):
        ...
        for x, Result in enumerate(obj):
            ...
            for key in keys.split():
                ...
                maximum = max(maximum+2, len(key)+2)
                if maximum > maxsize: maximum = maxsize
                Content = Content + "%s" % str(value)[:maxsize-2].ljust(maximum)
            self.Log(Content)
            if x > 30:
                break
        self.Log(" ")
    ...
    def findID(self, Movie, TVShow, Year, Season, IMDB, SearchTitle, SearchString):
        ...
        for x in range(0,len(SearchElements)):
            ...
            for search in [ fromBegin, fromEnd]:
                if len(search):
                    url     = "http://legendas.tv/legenda/sugestao/" + urllib.parse.quote_plus(search)
            ...
            for thread in self.RegThreads:
                ...
                try:
                    ...
                    contents = simplejson.loads(str(thread.join().read(), 'utf-8', errors='ignore'))
        ...
        if not len(allResults):
            ...
            Response = self._urlopen("http://minister.legendas.tv/util/busca_titulo/" + urllib.parse.quote_plus(SearchString)).read()
            Response =  simplejson.loads(str(Response, 'utf-8', errors='ignore'))
            # Load the results
            # Parse and filter the results
            self.Log("Message: Searching for movie/tvshow list with term(s): [%s]" % SearchString)
            for R in Response:
...
#        Response = self._urlopen("http://minister.legendas.tv/util/carrega_legendas_busca/page:%s/id_filme:%s" % (Page, MainID["id"]))
        Response = self._urlopen("http://legendas.tv/legenda/busca/-/-/-/%s/%s" % (Page, MainID["id"])).read().decode('utf-8')
        ...
        if not re.findall(regex_1, Response, re.IGNORECASE | re.DOTALL):
            ...
            for x, content in enumerate(re.findall(regex_1, Response, re.IGNORECASE | re.DOTALL), start=1):
                ...
                deuladeu = re.search("Português-BR", lang)
    ...
    def Download(self, url):
        Response = self._urlopen(url).read().decode('utf-8')
